...
const FaqAccordionItem: React.FC<{ item: FaqItem }> = ({ item }) => {
    ...
    return (
        <div className="border-b border-gray-300 dark:border-gray-700 py-4">
            ...
            >
                <h3 className={`flex-1 text-xl font-bold ${isOpen ? 'text-theme-red' : ''}`}>{item.question}</h3>
                <div className={`w-8 h-8 flex items-center justify-center rounded-full bg-theme-red text-white transition-transform duration-300 ${isOpen ? 'rotate-90' : 'rotate-0'}`}>
            ...
            <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-96 mt-4' : 'max-h-0'}`}>
                <p className="text-light-text-muted dark:text-dark-text-muted leading-relaxed">{item.answer}</p>
            </div>
        </div>
    );
};
...
const Faq: React.FC = () => {
    return (
        <section id="faq" className="py-24 flex flex-col md:flex-row items-start gap-16">
            {/* Left Side: Sticky Header */}
            <div className="md:w-1/3 flex flex-col items-start gap-6 md:sticky md:top-24"> 
                <h2 className="text-4xl lg:text-5xl font-extrabold leading-tight">Frequently Asked Questions</h2>
                <p className="text-light-text-muted dark:text-dark-text-muted leading-relaxed">
                    Can't find the answer you're looking for? Please chat with our friendly team.
                </p>
                <a href="#contact" className="mt-4 px-6 py-3 rounded-full border-2 border-light-text dark:border-dark-text font-semibold hover:bg-light-text hover:text-white dark:hover:bg-dark-text dark:hover:text-dark-bg transition-colors">
                    Ask More
                </a>
            </div>

            {/* Right Side: Scrollable List */}
            {/* Added: h-[400px], overflow-y-auto, pr-4 */}
            <div className="md:w-2/3 mt-8 md:mt-0 h-[360px] overflow-y-auto pr-4 custom-scrollbar">
